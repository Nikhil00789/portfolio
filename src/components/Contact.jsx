...
const inputClass = (hasError) =>
  `w-full bg-surface border rounded-lg px-4 py-3 font-body text-sm text-white placeholder-txt-muted/50 focus:outline-none focus:ring-1 transition-all duration-200 ${
    hasError
...
export default function Contact() {
  ...
  const handleSubmit = (e) => {
    ...
    setStatus("loading");
    // Simulated submission — replace with Formspree/EmailJS for real email
    setTimeout(() => {
      setStatus("success");
          ...
          <div className="flex flex-col gap-6">
            <div className="reveal">
              ...
              <div className="flex flex-col gap-3">
                {contactLinks.map(({ icon: Icon, label, href, display }) => (
                  ...
                  >
                    <div
                      className="w-10 h-10 rounded-lg border border-border bg-surface flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:border-accent/40"
                      style={{ "--hover-bg": "rgba(0,212,255,0.08)" }}
                    >
                      <Icon size={16} style={{ color: "#00d4ff" }} />
                    </div>
                    <div>
                      <p className="font-mono text-xs text-txt-muted mb-0.5">{label}</p>
                      <p className="font-body text-sm text-white transition-colors duration-200 group-hover:text-accent"
                        style={{ "--accent": "#00d4ff" }}
                        onMouseEnter={e => e.currentTarget.closest('a') && (e.currentTarget.style.color = "#00d4ff")}
            ...
            <div className="reveal border border-border bg-bg rounded-xl p-5 font-mono text-xs">
              <div className="flex items-center gap-1.5 mb-3">
                ...
                <span className="ml-2 text-txt-muted text-xs">nikhil.sh</span>
              </div>
            ...
            <div className="border border-border bg-card rounded-2xl p-6">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
                <div>
                  ...
                  <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="John Doe" className={inputClass(errors.name)} />
                ...
                <div>
                  ...
                  <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="john@example.com" className={inputClass(errors.email)} />
                ...
                <div>
                  ...
                  <textarea name="message" value={form.message} onChange={handleChange} rows={5} placeholder="Tell me about your project or opportunity..." className={inputClass(errors.message) + " resize-none"} />
                ...
                {status === "success" && (
                  <div className="flex items-center gap-2 p-3 rounded-lg" style={{ backgroundColor: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.3)" }}>
                    ...
                    <p className="font-mono text-xs text-green-400">Thanks! I'll get back to you soon.</p>
                  </div>
                )}

                <p className="font-mono text-xs text-center mt-1" style={{ color: "rgba(148,163,184,0.6)" }}>
                  * Connect to real email via{" "}
                  <a href="https://formspree.io" target="_blank" rel="noreferrer" style={{ color: "rgba(0,212,255,0.7)" }} onMouseEnter={e => e.currentTarget.style.color = "#00d4ff"} onMouseLeave={e => e.currentTarget.style.color = "rgba(0,212,255,0.7)"}>Formspree</a>
                  {" "}or EmailJS.
                </p>
              </form>
            </div>
